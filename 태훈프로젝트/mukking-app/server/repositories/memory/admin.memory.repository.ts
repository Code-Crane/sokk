...
export const memoryAdminRepository: AdminRepository = {
  ...
  async createAuditLog(input: CreateAdminAuditLogInput) {
    const log: AdminAuditLog = {
      ...
      targetSanctionId: input.targetSanctionId,
      targetPostId: input.targetPostId,
      targetChatRoomId: input.targetChatRoomId,
      aal: input.aal,
  ...
  async listAuditLogs(filter: AdminAuditLogFilter = {}) {
    const logs = Array.from(adminAuditLogs.values())
      ...
      .filter((log) =>
        filter.targetUserId ? log.targetUserId === filter.targetUserId : true
      )
      .filter((log) =>
        filter.targetReportId ? log.targetReportId === filter.targetReportId : true
      )
